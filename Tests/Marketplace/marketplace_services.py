...
class Pack(object):
    ...
    def copy_integration_images(self, production_bucket, build_bucket, current_commit_hash, previous_commit_hash,
        ...
        try:
            ...
            for file in current_commit.diff(previous_commit):
                ...
                if file.a_path.startswith(f"{os.path.join(PACKS_FOLDER, self._pack_name)}") and \
                        is_integration_image(image_name):
    ...
    def copy_author_image(self, production_bucket, build_bucket, current_commit_hash, previous_commit_hash,
        ...
        """
        err_msg = f"Failed uploading {self._pack_name} pack author image."

        if self.support_type == Metadata.XSOAR_SUPPORT:
            # use default Base pack image for xsoar supported packs
            logging.info((f"Skipping uploading of {self._pack_name} pack author image and use default "
                          f"{GCPConfig.BASE_PACK} pack image"))
            return True
        ...
        try:
            ...
            author_images_paths = [file.a_path for file in current_commit.diff(previous_commit) if
                                   file.a_path.startswith(os.path.join(PACKS_FOLDER, self._pack_name)) and
                                   os.path.basename(file.a_path) == Pack.AUTHOR_IMAGE_NAME]
            if not author_images_paths:
                ...
                if build_author_image_blob.exists():
                    ...
                    logging.error(f"Found changed/added author image in content repo but image does not exist in build"
                                  f" bucket in path {build_author_image_path}.")
                    return False
